...
float4 BloomPerfilterPassFragment(Varyings input) : SV_TARGET
{
...
float4 BloomPerfilterFirefliesPassFragment(Varyings input) : SV_TARGET
{
    float3 color = 0.0;
    float weightSum = 0.0;
    float2 offset[] = {
    ...
    for(int i = 0; i < 5; i++)
    {
        float3 c = GetSource(input.screenUV + offset[i] * GetSourceTexelSize().xy * 2.0).rgb;
