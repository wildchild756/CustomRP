...
#define MAX_SHADOWED_DIRRECTIONAL_LIGHT_COUNT 4
...
CBUFFER_START(CustomShadows)
    ...
    float4x4 _DirectionalShadowMatrices[MAX_SHADOWED_DIRRECTIONAL_LIGHT_COUNT * MAX_CASCADE_COUNT];
...
float GetBakeShadow(ShadowMask mask, int channel)
{
    float shadow = 1.0;
...
float GetBakeShadow(ShadowMask mask, int channel, float strength)
{
    if(mask.always || mask.distance)
    {
        return lerp(1.0, GetBakeShadow(mask, channel), strength);
    }
    return 1.0;
}

float MixBakeAndRealtimeShadows(ShadowData global, float shadow, int shadowMaskChannel, float strength)
{
    float baked = GetBakeShadow(global.shadowMask, shadowMaskChannel);
...
float GetDirectionalShadowAttenuation(DirectionalShadowData directional, ShadowData global, Surface surfaceWS)
{
    ...
    {
        shadow = GetBakeShadow(global.shadowMask, directional.shadowMaskChannel, abs(directional.strength));
    }
    else
    {
        shadow = GetCascadeShadow(directional, global, surfaceWS);
        shadow = MixBakeAndRealtimeShadows(global, shadow, directional.shadowMaskChannel, directional.strength);
...
float GetOtherShadowAttenuation(OtherShadowData other, ShadowData global, Surface surfaceWS)
{
    ...
    {
        shadow = GetBakeShadow(global.shadowMask, other.shadowMaskChannel, abs(other.strength));
    }
    else
    {
        shadow = GetOtherShadow(other, global, surfaceWS);
        shadow = MixBakeAndRealtimeShadows(global, shadow, other.shadowMaskChannel, other.strength);
    }
    return shadow;
}
